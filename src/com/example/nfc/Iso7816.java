...
public class Iso7816 {
	...
	public final static class Tag {
		...
		public Response getMoney()
		{
			...
			Log.d("from getmoney", "respone = " + response.toString() + "__"
					+ "money = " + money + "__" + response.getSw1() + "__" + response.getSw2());
		...
		public Response getData()
		{
			final byte[] cmd = { (byte) 0x80, // CLA Class
					...
					(byte) 0x36, // P2 Parameter 2
					(byte) 0x00, // Le
			};
			
			Response response = new Response(transceive(cmd));
			Log.d("Tag from nfc readRecord_index", "get data = "+ response.toString() 
					+ "__" + response.getSw1()+ "__" + response.getSw2());
			
			return response;
		}
		
		//added by yh
		public Response readRecordAll()
		{
			final byte[] cmd = { (byte) 0x00, // CLA Class
					(byte) 0xB2, // INS Instruction
					(byte) 0x00, // P1 Parameter 1
					(byte) 0x05, // P2 Parameter 2
					(byte) 0x00, // Le
			};
			
			Response response = new Response(transceive(cmd));
			Log.d("Tag from nfc readRecord_index", "record = "+ response.toString() 
					+ "__" + response.getSw1()+ "__" + response.getSw2());
		...
		/**
		 * VERIFY command 
		 * for wuhantong 0x6982 安全状态不被满足 ,send this command before readRecord  
		 * @return
		 */
		public Response Verify(byte Lc, byte[] key)
		{
//			final byte[] cmd = { (byte) 0x00, // CLA Class
//					(byte) 0x20, // INS Instruction
//					(byte) 0x00, // P1 Parameter 1
//					(byte) 0x00, // P2 Parameter 2
//					(byte) 0x03, // L blow data segment length
//					(byte) 0x12, // data segment
//					(byte) 0x34, // data segment
//					(byte) 0x56, // data segment
//			};
			ByteBuffer bytes = ByteBuffer.allocate(4+1+key.length);
			bytes.put((byte)0x00) //CLA
			     .put((byte)0x20) //INS
			     .put((byte)0x00) //P1
			     .put((byte)0x00) //P2
			     .put(Lc)   //Lc
